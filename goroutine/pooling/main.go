...
// Close implements the io.Closer interface so dbConnection can be managed by the pool. Close performs any
...
func main() {
    ...
    for query := 0; query < maxGoroutines; query++ {
        // Each goroutine needs its own copy of the query value else they will all be sharing the same query
        // variable.
        go func(q int) {
            performQueries(q, p)
            wg.Done()
